...
public class PasswordRepositoryImpl implements PasswordRepository {
    ...
    public void saveToken(ResetToken resetToken) {
        ...
        template.update("INSERT INTO reset_tokens (email, username, token, expiry_date) VALUES (?, ?, ?, ?);", resetToken.getEmail(), resetToken.getUsername(), resetToken.getToken(), resetToken.calculateExpiryDate(ResetToken.EXPIRATION_MS));
    }

    @Override
    public ResetToken findByToken(String token) {
        JdbcTemplate template = new JdbcTemplate(dataSource);
        ResetToken resetToken = template.queryForObject("SELECT email, username, token, expiry_date FROM reset_tokens WHERE token = ?", (resultSet, rowNum) -> {
